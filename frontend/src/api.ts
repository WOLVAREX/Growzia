...
export const api = {
  ...
  updateProfile: (username:string) => request<{user:User}>('/auth/profile',{method:'PATCH',body:JSON.stringify({username})}),
  changePassword: (currentPassword:string,newPassword:string) => request<{changed:boolean}>('/auth/password',{method:'PATCH',body:JSON.stringify({currentPassword,newPassword})}),
  ...
  adminFinance: (period:'all'|'week'|'month'='all') => request<any>(`/admin/finance?period=${period}`),
  adminUsers: () => request<any>('/admin/users?limit=100'),
  adminOrders: () => request<any>('/admin/orders?limit=100'),
  initiateOrder: (id:string) => request<any>(`/admin/orders/${encodeURIComponent(id)}/initiate`,{method:'POST'}),
  providerCheck: (id:string) => request<any>(`/admin/orders/${encodeURIComponent(id)}/provider-check`),
