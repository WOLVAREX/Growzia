...
export default function AdminEmailTools({ users }: { users: any[] }) {
  ...
  const [senderIds, setSenderIds] = useState<{id:string;value:string;label:string;isActive:boolean}[]>([]);

  useEffect(() => {
    void Promise.all([api.providerAlerts(), api.processingWindow()]).then(([alerts, window]) => {
      setNumbers((alerts.numbers || []).join(', '));
      setSenderIds(alerts.senderIds || []);
      setSenderId(alerts.senderId || alerts.senderIds?.[0]?.id || '');
  ...
  return <div className="email-tools">
    ...
    <label>Nena sender ID{senderIds.length ? <select value={senderId} onChange={event => setSenderId(event.target.value)}>{senderIds.map(sender => <option key={sender.id} value={sender.id}>{sender.value}{sender.label && sender.label !== sender.value ? ` — ${sender.label}` : ''} · {sender.id}</option>)}</select> : <input value={senderId} onChange={event => setSenderId(event.target.value)} placeholder="xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx"/>}<small>Nena requires the UUID assigned to an active sender ID. Sender shown above is selected from Nena.</small></label>
