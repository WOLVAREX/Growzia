...
type ProviderRow = { name: string; providerServiceId: string; baseKesPer1000: number; rawRate: number; rawCurrency: string; min: number; max: number };
type CatalogRow = { canonicalKey: string; platformId: string; serviceType: string; bwm: ProviderRow | null; cheapgains: ProviderRow | null; winner: { providerCode: string; baseKesPer1000: number; sellKesPer1000: number; name: string } | null };
...
export default function AdminCatalogEnhanced({ rows, refresh }: { rows: CatalogRow[]; refresh: () => void }) {
  ...
  useEffect(() => {
    api.providerSettings().then(result => setDisabled(new Set(result.providerServices || []))).catch(() => setMessage('Could not load provider controls.'));
  }, []);
